...
void signal_handler(int signum) {
    if (signum == SIGUSR1) {
		printf("Signal received in camera program.\n");
        video_capture_save_raw(); // Capture image when signal is received
	}
...
{
	// struct device dev; // made into global variable to make cleanup possible
    ...
    //     //         video_capture_save_raw(); // Call the function
    ...
    if (pid == 0) { // Child process
        if (argc > 1 && std::string(argv[1]) == "capture") {
            std::cout << "Child process ready to capture." << std::endl;
            while (true) {
