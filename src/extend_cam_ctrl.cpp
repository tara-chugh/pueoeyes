...
// PYBIND11_MODULE(capture_module, m) {
// 	m.def("capture raw", &video_capture_save_raw, "Callback");
...
int v4l2_core_save_data_to_file(const char *filename, const void *data, int size)
{
	...
	if ((fp = fopen(filename, "wb")) != NULL)
	{
		...
		if (fsync(fileno(fp)) || fclose(fp))
			...
			printf("V4L2_CORE (with updates): saved data to %s\n", filename);
			raw_to_png_image(filename);
			printf("Done.\n");
		}
	}
	else
...
void raw_to_png_image(std::string filename)
{
    ...
    // std::string::c_str(): Returns a pointer to an array that contains a null-terminated sequence of characters
        // C strings are arrays of characters
    // rb = read BINARY file
    FILE* file = fopen(filename.c_str(), "rb");
	unsigned short *data = new unsigned short[h*w];
    ...
    // ushort = 2 bytes (16bits), sizeof() returns size in bytes
    // Reads an array of 'count' elements, each one with a size of "size" bytes, from the 'stream' and stores them in the block of memory specified by 'ptr'. 
    fread(data, sizeof(unsigned short), h * w, file);
    fclose(file);

	cv::Mat bayer(h, w, CV_16U, data); // row, column, type=16-bit unsigned integers


	cv::Mat BGR16;
	cv::cvtColor(bayer, BGR16, cv::COLOR_BayerGR2BGR);
	cv::Mat channels[3];
	cv::split(BGR16, channels);
	channels[0].convertTo(channels[0], CV_32F);
	channels[1].convertTo(channels[1], CV_32F);
	channels[2].convertTo(channels[2], CV_32F);
	cv::sqrt(channels[0], channels[0]);
	cv::sqrt(channels[1], channels[1]);
	cv::sqrt(channels[2], channels[2]);

	channels[0] *= 2300;
	channels[1] *= 1300;
	channels[2] *= 1700;

    // Each element of the output array will be a concatenation of the elements of the input arrays, where elements of i-th input array are treated as mv[i].channels()-element vectors.
    cv::merge(channels, 3, BGR16);

    // convert BGR16 to type CV_16U, store in BGR16
    BGR16.convertTo(BGR16, CV_16U);

    // COLOR CORRECTION STEP #2 (use 99.9% threshold for overflow)
	cv::Mat mask_blue = (BGR16.col(0) >= 65433.501);
    cv::Mat mask_green = (BGR16.col(1) >= 60580.359);
    cv::Mat mask_red = (BGR16.col(2) >= 65466.468);

    for (int i = 0; i < h / 2; ++i) {
        if (mask_green.at<uchar>(i, 0) || mask_blue.at<uchar>(i, 0) || mask_red.at<uchar>(i, 0)) {

            // typedef Vec<ushort, 3> cv::Vec3w
            BGR16.row(i) = cv::Vec3w(65535, 65535, 65535);
        }
    }
	

    // Save result as 16-bit PNG
	printf("Saved PNG image to to %s\n", (filename.substr(0, filename.size()-4) + ".png").c_str());
    cv::imwrite(filename.substr(0, filename.size()-4) + ".png", BGR16);
}
...
//	channels[2] *= 1700;
  ...
  //  BGR16.convertTo(BGR16, CV_16U);
    ...
    // COLOR CORRECTION STEP #2 (use 99.99% threshold for overflow)
    // cv::Mat mask_blue = (BGR16.col(0) >= 65492.4501);
//cv::Mat mask_green = (BGR16.col(1) >= 60634.9359);
//cv::Mat mask_red = (BGR16.col(2) >= 65525.4468);
